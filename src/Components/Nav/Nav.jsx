...
function Nav() {
    const [selectedCity, setSelectedCity] = useState('Xajiqurbonov.A');
    const cities = [
            ...
            <nav className='w-[100%] flex items-center justify-between px-[23px] py-[12px] bg-white'>
              <div className='flex items-center gap-[33px] '>
                <div className='flex items-center gap-[17px]'>
                  <img src={Amaki} alt="" />
                    <Dropdown options={cities} optionLabel="name" placeholder="Xajiqurbonov.A" className="w-[180px] bg-white" itemTemplate={(option) => <div className="flex items-center gap-2">{option.icon}<span>{option.name}</span></div>} />
             </div>
