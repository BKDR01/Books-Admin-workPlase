...
export default function DeleteNews() {
  ...
  const fetchNews = async (lang) => {
    setLoading(true);
    try {
      const langs = lang === "ALL" ? ["UZ", "RU", "EN"] : [lang];
      const allNews = (
        await Promise.all(
          langs.map((l) =>
            getAllAdminNews({ page: 1, limit: 1000, language: l, t: Date.now() })
          )
        )
      ).flatMap((res) => res?.data?.data || []);

      console.log("📦 API’dan kelgan ma’lumot:", allNews);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await Promise.all(relatedNews.map((n) => deleteNews(n.id)));
